...
from bot.config import (
    ...
    MARKETING_BASE_URL,
    STATIC_URL,
    EMOTION_API_URL,  # Nuevo endpoint
    UPLOADS_DIR,
    STATIC_DIR,
    logger,
)
...
async def handle_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if state == "awaiting_image":
            # Procesar imagen MRI (lógica existente)
            try:
            ...
            if img_tumor_url and diagnosis == "Tumor detectado":
                try:
                    if img_tumor_url.startswith("data:image"):
            ...
            # Procesar imagen para reconocimiento de emociones
            try:
                with open(file_path, "rb") as f:
                ...
                # CAMBIO PRINCIPAL: Enviar los datos a n8n para el correo
                try:
                    ...
                    n8n_data = {
                        ...
                        "chat_id": str(chat_id),
                        "emotion": "Ver en imagen"  # Agregar campo emotion
                    }
                    ...
                    # No interrumpimos el flujo, solo loggeamos el error
    ...
    if query.data == "main_menu":
        ...
        try:
            ...
            keyboard = [
                ...
                for i, img in enumerate(images[:5])  # Limitar a 5 imágenes
            ]
        ...
        try:
            # Usar webhook de n8n para reprocesamiento
            ...
            # El resultado llegará por correo, solo confirmar que se procesó
